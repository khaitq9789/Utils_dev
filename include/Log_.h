...
#if LOG_ENABLED
// Con trỏ tệp log (chỉ được tạo nếu LOG_ENABLED = 1)
static std::shared_ptr<std::ofstream> log_file_ptr;

inline void initialize_log_file() {
    if (!log_file_ptr) {
        log_file_ptr = std::make_shared<std::ofstream>(LOG_FILE_PATH);
